...
- **Dump generation pipeline** — `dump-generation/generate.sh` is invoked as a daily Toolforge job (see `toolforge/jobs.yaml`). It:
  1. Reads `/public/dumps/public/wikidatawiki/entities/latest-all.json.gz` (Toolforge's NFS-mounted Wikimedia dump path). This file weighs more than 150GB and contains more than 100 million items so it's critical that the filtering step is kept fast and optimized.
  2. Decompresses with `pigz`, `grep`s for `P625":` to drop non-place items, and fans out across cores via `parallel --pipe` to a single `jq` filter that performs the claim presence/absence check (P625 present; P585/P376/P580/P571/P1619/P582/P576/P3999 absent) and Feature conversion in one parse, producing **GeoJSONSeq** (`places.geojsonl`, RFC 8142 newline-delimited GeoJSON Features). The `jq` filter walks `claims.P625`, emits one Feature per coordinate pair, and swaps `[lat, lon]` → `[lon, lat]` for GeoJSON order. Filter rules are still being tightened (see `#TODO` lines in `generate.sh`).
  3. Runs `ogr2ogr` against `places.geojsonl` to produce `.fgb` (FlatGeobuf) and `.parquet` (GeoParquet); `.geojson` (a single FeatureCollection) is only produced in `--test` mode because the file is too large in production.
