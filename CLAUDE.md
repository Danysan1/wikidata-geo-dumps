...
- **Dump generation pipeline** — `dump-generation/generate.sh` is invoked as a daily Toolforge job (see `toolforge/jobs.yaml`). It:
  ...
  2. Greps `P625":`, pipes through `wikibase-dump-filter` (npm dep) with `--simplify --omit aliases --claim 'P625&~P585&~P376&~P580&~P571&~P1619&~P582&~P576&~P3999'`, and immediately pipes the result through a `jq` filter to produce **GeoJSONSeq** (`places.geojsonl`, RFC 8142 newline-delimited GeoJSON Features) in one pipeline — no intermediate NDJSON file. The claim filter requires P625 (coordinates) present and excludes items with P585/P376/start/end-date qualifiers — these rules are still being tightened (see `#TODO` lines in `generate.sh`). The `jq` filter walks `claims.P625`, emits one Feature per coordinate pair, and swaps `[lat, lon]` → `[lon, lat]` for GeoJSON order.
