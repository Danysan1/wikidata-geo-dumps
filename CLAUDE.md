...
This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository. Keep it up to date: after any change to architecture, pipeline steps, memory constraints, or tooling, update the relevant section here. Do not duplicate information that is obvious from reading the source files.

## What this project is

Pipeline that produces geographic dumps from Wikidata & web service that exposes them. It filters the full Wikidata JSON dump for entities with coordinates (P625), makes sure that they are currently existing features, then converts the result to GeoJSONSeq, FlatGeoBuf, and GeoParquet. The generated files are served as static downloads alongside a small landing page. Deployed to Wikimedia Toolforge as the tool `tool-wikidata-geo-dumps`.
...
- **Dump generation pipeline** — `dump-generation/generate.sh` runs as a daily Toolforge job. It decompresses the NFS-mounted Wikidata dump (150 GB+, 100 M+ items), pre-filters with `grep -F` for speed, then fans out to `jq -f dump-generation/filter.jq` in parallel. The filter keeps only items with P625 coordinates and no claims indicating the feature is historical/ended, and emits one GeoJSONSeq feature per item (`places.geojsonl`) with `id`, `name`/`name:en`, and `commons` properties. Filter rules are still being tightened — see the `#TODO` block at the top of `filter.jq`. `ogr2ogr` converts the GeoJSONSeq to FlatGeoBuf. `dump-generation/geojsonseq-to-parquet.py` converts the GeoJSONSeq to GeoParquet by streaming it line-by-line in batches (100 k rows at a time) using `pyarrow` directly — this keeps peak memory O(batch) rather than O(file); **do not replace this with `geopandas.read_file().to_parquet()` as it loads the whole file into RAM and OOMs at the 6 GiB job limit**. Output lands in `$TOOL_DATA_DIR/dist/dumps/<date>/`. Finally, `dump-generation/generate-index.py` writes `$TOOL_DATA_DIR/dist/dumps/index.json` for the web UI.
...
`Procfile` defines two process types: `web` (static server) and `generate` (dump pipeline). `toolforge/jobs.yaml` schedules `generate` daily with `cpu: 2`, `mem: 6Gi`, `timeout: 28800`.
...
- Use Codex CSS custom properties (`--color-*`, `--spacing-*`, `--font-size-*`, etc.) for all colors, spacing, and typography. Do not introduce custom color values.
