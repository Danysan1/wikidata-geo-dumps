...
if [ -f "$PLACES_NDJSON_PATH" ]; then
    ...
    total_lines=$(wc -l "$SOURCE_DUMP")
    echo "Filtering $PLACES_NDJSON_PATH from only the first 100'000 lines out of $total_lines from $SOURCE_DUMP"
    ...
    #time cat $SOURCE_DUMP | gzip -d | grep 'P625":' | wikibase-dump-filter "${filter_options[@]}" > "$PLACES_NDJSON_PATH"
fi
#endregion
